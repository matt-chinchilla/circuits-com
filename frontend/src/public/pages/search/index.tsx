...
// MIRRORS of api/app/services/search_service.py PARTS_LIMIT / SECTION_LIMIT
// (the mirrored-homes convention): the server caps each section there; these
// literals only decide when `total` reads as a truncation floor.
const PARTS_LIMIT_MIRROR = 20;
const SECTION_LIMIT_MIRROR = 12;
...
export default function SearchPage() {
  ...
  // Server `total` sums CAPPED sections \u2014 when any section hits its cap the
  // count is a truncation floor, not exhaustive, so the copy must say
  // "top N matches" (review finding, 2026-08-21).
  const atCap =
    results != null &&
    (results.parts.length >= PARTS_LIMIT_MIRROR ||
      results.categories.length >= SECTION_LIMIT_MIRROR ||
      results.suppliers.length >= SECTION_LIMIT_MIRROR ||
      results.manufacturers.length >= SECTION_LIMIT_MIRROR);
