...
import { BOARD_KEYS, VIEW_MODE_KEYS } from '@public/components/kicad/board3d/shortcuts';
...
describe('the viewer claims each single key once', () => {
  it('no key is shared between the view tabs, the 3D controls and the older shortcuts', () => {
    const claimed = [...Object.values(VIEW_KEY), ...Object.values(BOARD_KEYS), ...Object.values(VIEW_MODE_KEYS), '/', 'Escape'];
    expect(new Set(claimed).size).toBe(claimed.length);
  });

  it('every key is a single lowercase character, as callers compare e.key.toLowerCase()', () => {
    for (const key of [...Object.values(VIEW_KEY), ...Object.values(BOARD_KEYS), ...Object.values(VIEW_MODE_KEYS)]) {
