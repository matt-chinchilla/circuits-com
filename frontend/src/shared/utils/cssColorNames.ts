// Nearest-CSS-color-name lookup for arbitrary brand hexes (admin sponsor form +
// Platinum swatch picker use this to label extracted palette swatches with a
// human-friendly name, e.g. "Dark Slate Blue"). Table = the CSS Color Module
// keyword set (147 CSS3 keywords + `rebeccapurple` from Level 4 = 148 total),
// with duplicate-hex aliases skipped in favor of one display name per hex:
// aqua/cyan -> Cyan, fuchsia/magenta -> Magenta, every `*gray`/`*grey` pair ->
// the "gray" spelling. That leaves 139 unique-hex rows below.
...
interface CssColorEntry {
  name: string;
  hex: string;
}

const CSS_COLORS: CssColorEntry[] = [
...
/** Nearest named CSS color to `hex` — exact (case-insensitive) match first, else min sRGB Euclidean distance. */
export function nearestCssColor(hex: string): NamedColor | null {
  const normalized = safeHexColor(hex);
  if (!normalized) return null;
  const lower = normalized.toLowerCase();
  const exact = CSS_COLORS.find((c) => c.hex === lower);
  if (exact) return { name: exact.name, exact: true };

  const [r, g, b] = parseHex(lower);
  let best = CSS_COLORS[0];
