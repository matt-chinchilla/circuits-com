...
- **`<CircuitTraces variant="full" />` is hero-ONLY** — once, in `BackdropLayer` (App.tsx, above `<Routes>`, persistent across nav). Others use `variant="static"` EXCEPT Silver `SilverPartners` (2nd `full`, IO-gated + filter-free, 60fps). `full`=14 SMIL loops+6s draw+IO bookkeeping; a 2nd on a non-hero route breaks the perf invariant.
...
- **Admin login = v13 two-panel (`pages/login/`) + recovery** — creds add `demo`/`demo` (admins seeded w/ emails, migration 015). `LoginPage.module.scss` ports the design under ONE hashed `.authRoot{:global{…}}` (literal classnames, `auth*` keyframes, no global `*`/`body` resets). Reset-link origin MUST be `settings.APP_BASE_URL`, NEVER `request.base_url` (poisoning).
