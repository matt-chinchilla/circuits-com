...
- **PreferredPartnersBanner = the v15 partner directory** (`pages/category/components/PreferredPartnersBanner.tsx`, mounted where `featured.length > 0`; replaces v11–v14 CategorySponsorBanner). Bicameral grid: left identity over right rail of 5-col rows (medallion · Company + `#{s.rank}` chip + scheme-prepended site link · Sales Contact `contact_name` · Phone+⧉ · Email+⧉). `<CircuitTraces variant="static" />`; always expanded, `max-height:360px`. NO flashlight (perf). Chip `onKeyDown` gates on `e.target === e.currentTarget`. Phones render via `@shared/utils/phone.formatPhone` (US→`(NNN) NNN-NNNN`; intl `+CC`→spaces) — DB normalized to match. Add/remove: `POST /api/admin/category-suppliers/{feature,unfeature}` OR `POST /api/admin/sponsors/` tier=Featured (auto-features).
- **Category `.suppliers` = the category's OWN rows ONLY (no child rollup)** — `get_category_by_slug` uses `cat_ids=[category.id]`, NOT children, so a child's Featured Subcategory Sponsor does NOT surface on the PARENT banner/TopPartners (the 2026-06-02 "deleted Oneonta still shows on PMICs via the ldo-regulators rollup" bug). PARTS still roll up (`_build_popular_parts`); suppliers do not.
