...
- **Sponsorships are the SINGLE source of truth (2026-06-03 migration 011)** — the banner reads ACTIVE `sponsors`-table rows, tier-ordered, via `get_category_partners` (the `/partners` endpoint — see below), NOT a join flag. `CategorySupplier.is_featured`/`rank` were DROPPED; the join is a pure association (cascade only). /admin/sponsors and the banner are 1:1 — deleting a sponsor removes the company, no side-effect. GONE: `category-suppliers/{feature,unfeature}` + `_upsert_category_supplier_featured`/`_unfeature_after_delete`.
- **Sponsor tier ↔ placement matrix** — Category (top-level, `parent_id IS NULL`) = **Featured** only; Subcategory (child) = **Platinum/Gold** only; Keyword = **Silver/Gold/Platinum** (Silver is keyword-only). Enforced 3 ways: a Postgres TRIGGER `sponsor_tier_placement_check` (true DB-level — rejects raw SQL too), `_validate_tier_placement` → 422, and the admin form button-gating (`choosePlacement(p, keepTier?)`; tier-select flip is matrix-aware). DB ACID also: `UNIQUE(supplier_id, category_id)` + `UNIQUE(supplier_id, keyword)` → a company sponsors any category/keyword at most once (NULLs SQL-distinct → ∞ keywords + the ≤15/≤75 caps fall out of the taxonomy size); duplicate POST → 409 (IntegrityError-caught).
...
- **Tests use SQLite via `Base.metadata.create_all`** — SQLite ignores `String(N)` length AND CHECK. For length contracts assert on metadata: `Model.__table__.c.col.type.length >= N`. `conftest.py` disposes the in-memory engine PER TEST (fresh connection) — a shared StaticPool connection leaked SQLite type state → order-dependent UUID-as-float seed-test crashes; don't revert to one shared connection.
...
- **PreferredPartnersBanner** (`pages/category/components/PreferredPartnersBanner.tsx`) — bicameral grid: left identity + right rail of 5-col rows (medallion · Company + `#{i+1}` position chip + site link · `contact_name` · Phone+⧉ · Email+⧉). `variant="static"`, always expanded, NO flashlight (perf). Phones via `@shared/utils/phone.formatPhone`. Mounted by the thin `CategoryPartnersBanner.tsx` wrapper INSIDE CategoryPage (top of `.contentWide`, BELOW the breadcrumb + sticky sub-nav — NOT in PublicLayout, which would render it above the nav; hookify-guarded).
- **The banner is a TOP-LEVEL-category artifact via `GET /api/categories/{slug}/partners`** (2026-06-05) — the wrapper derives the top-level slug from the URL (a child slug resolves to its parent in `get_category_partners`), so the SAME banner shows on the parent + every subpage. `CategoryDetailResponse` NO LONGER has `suppliers` (`SponsorBlock`'s `category.sponsor` stays). Session memo `@shared/services/partnersMemo.ts` (keyed by resolved top-level slug, cleared by `bustSponsorCaches`) → sync render, no pop-in. `/partners` = `no-cache` + content-hash ETag/304, SW-cached in `api-categories`.
- **Subcats are NESTED `/category/:parent/:child`; flat `/category/:slug` redirects to nested** — `App.tsx` has BOTH routes → one `CategoryPage`. The canonical redirect MUST be an EFFECT (`navigate(...,{replace})`), NOT a render `<Navigate>` — `PublicLayout`'s `<ErrorBoundary key={pathname}>` drops the rendered child's nav-effect on the keyed remount (empty page, frozen URL — 2026-06-03). The `?p`-reset effect must bail while redirecting (`if (needsCanonicalRedirect) return`). URLs via `@shared/utils/categoryPath(slug, parentSlug?)`; sitemap nested for children.
- **Two banner surfaces, ONE source (`sponsors` table)** — Preferred Partners (top-level, multi-row, tier-ordered, via `/partners`) + `SponsorBlock` (newest single sponsor for THIS subcategory, `category.sponsor`). Both filter `or_(status=='Active', status.is_(None))` → can't drift. "Top Distributors"/`TopPartners` was deleted — the banner replaced it.
- **`get_all_categories` exposes `featured_suppliers: [{id,name}]`** + legacy `featured_supplier_name` (`[0].name`) per category+child — REAL Pydantic fields kept by `response_model`; admin `/admin/categories` tree shows them READ-ONLY (manage via /admin/sponsors).
...
- **Category-page entrances = opacity-only fades (no `x`/`y` translate)** — translate entrances re-fire per subcategory nav and read as "jitter"; `useEntrance` row-stagger deleted (2026-06-05, hookify-guarded).
...
- **`--reseed` can DEADLOCK when the deploy ships a heavy-DDL migration** — `deploy_reseed` recreates the api (entrypoint re-runs alembic+seed) THEN runs the clear `TRUNCATE`; a migration doing table DDL (DROP COLUMN / ADD CONSTRAINT / CREATE TRIGGER = AccessExclusiveLock) racing that TRUNCATE → `ERROR: deadlock detected`, the atomic TRUNCATE aborts, old data persists (2026-06-03 migration 011). Recovery (api now UP + idle, so no entrypoint race): re-run the clear+reseed alone — `ssh ... "docker exec circuits-com-db-1 psql -c 'TRUNCATE ...CASCADE' && docker exec circuits-com-api-1 python -m app.db.seed"`.
