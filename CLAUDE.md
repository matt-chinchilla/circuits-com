...
- **Hooks** (settings.json): PreToolUse blocks .env/lock edits, warns on `api/app/admin.py` (SQLAdmin ≠ prod admin); PostToolUse auto-runs (1) `tsc --noEmit` on .ts/.tsx/.scss edits, (2) `pytest` on .py edits, (3) `scss-lint.sh` (CLAUDE.md gotcha grep — 10 anti-patterns from this list, warns non-blocking), (4) `frontend-rebuild.sh` (background-detached `docker compose up -d --build frontend` on SCSS/TSX edits; flock-deduped so rapid edits coalesce)
- **Scripts** (`.claude/scripts/`): `scss-lint.sh`, `frontend-rebuild.sh` — both chmod'd executable, called by hooks
...
The glow filter uses `stdDeviation="0.5"` and a tight `-5%/110%` region; `@media (max-width: 768px) { .traceGroup { filter: none; } }` disables it on mobile (Blink/WebKit fall back to CPU raster during SourceGraphic invalidation; this caused 6–10s of frame drops until 2026-04-19). Alternate themes (steel/schematic/pcb) inherit `background-color: var(--theme-nav-bg)` on `.hero` so the board color matches the navbar — base theme keeps `$executive-blue`. `<CircuitTraces />` renders on 8 pages (Home, Category, Search, Contact, Join, About, KeywordSponsor, Part), not just `HeroSection` — any SVG/token change affects all of them.
