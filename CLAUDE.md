...
- **CategorySponsorBanner = parent-category sponsor surface** (shipped 2026-05-30, Concept A from design handoff v3 `category-sponsor/`) — full-width "Breakout Board" dark ENIG card rendered ABOVE the parts table in `category/index.tsx`, gated to `!loading && category && isParent && !activeSubInfo`. Identity block (medallion + company + blurb) left, 4 break-out pads right (Company / Contact / Phone / Email — P1-P4). Click-to-copy on phone+email via local `CopyChip` (timer-ref'd cleanup on unmount). CTA renders only when email OR website is present (don't fall back to `href="#"` — that scrolls the user to top on tap). Empty-state placeholder ALWAYS renders on parent pages so the open slot is discoverable. Data: `sponsor.{supplier_name,description,phone,email,contact_name}` — MVP equates supplier-level contact with the sales rep; future `Sponsor.rep_*` columns will split them. NO flashlight reveal (deferred); NO inline contact form state-machine (deferred). Sibling — NOT replacement — of `SponsorBlock` (which still owns the SUBCATEGORY sidebar slot).
