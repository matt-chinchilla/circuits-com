...
- **Admin image upload = base64 data-URL in DB, NO upload endpoint (2026-06-22)** — `Sponsor.image_url` + `Supplier.logo_url` widened `String(500)→Text` (migration 017). Shared `ImageUploadField` (`@admin/components`, drives BOTH sponsor + supplier forms) does file→`fileToDataUrl` (`@admin/utils/image`: `<canvas>` downscale longest-edge→256px→WebP/JPEG, ≤64KB, MIME/empty guards BEFORE `createObjectURL`, revoke in `finally`) OR paste-URL — both write the same `value` (typed `string | null`). Chosen because the **API container has NO volume mount** (disk uploads wiped each rebuild) but the DB has a volume; public `<img>` renders data-URLs natively. `Supplier.logo_url` (pre-existing col, now form-settable + admin list/detail thumbnail) feeds ALL tier boards — Gold/Silver fall back `image_url ?? logo_url` (Platinum already did).
