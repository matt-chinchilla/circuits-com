...
- **Sponsorships are the SINGLE source of truth (2026-06-03, migration 011)** — the Preferred Partners banner reads ACTIVE `sponsors`-table rows for the category (`category_service.get_category_by_slug.suppliers` + `get_all_categories.featured_suppliers`, tier-ordered), NOT a join flag. `CategorySupplier.is_featured`/`rank` were DROPPED; the join is now a pure association (powers Top Distributors). So /admin/sponsors and the category page are 1:1 — deleting a sponsor removes the company from the page, no side-effect to reverse. GONE: the `category-suppliers/{feature,unfeature}` endpoints + `_upsert_category_supplier_featured`/`_unfeature_after_delete`.
...
- **Two banner surfaces, ONE source (`sponsors` table)** — `PreferredPartnersBanner` = this category's active sponsorships (multi-row, tier-ordered); `SponsorBlock` = the newest single one. Both read `sponsors` filtered `or_(status=='Active', status.is_(None))`, so they can't drift — the old `is_featured`-vs-`sponsors` divergence ("Matt's Test invisible" / "deleted still shows") is structurally gone.
- **`get_all_categories` exposes `featured_suppliers: [{id,name}]`** (tier-ordered, from active sponsorships) + legacy `featured_supplier_name` (= `[0].name`) on every category + child. Both are REAL Pydantic fields (not stamped attrs) so `response_model` keeps them. The admin `/admin/categories` tree renders them READ-ONLY (no Unfeature button — manage via /admin/sponsors).
