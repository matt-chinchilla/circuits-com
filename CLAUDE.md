...
- **PreferredPartnersBanner = the v15 partner directory** (`pages/category/components/PreferredPartnersBanner.tsx`, mounted where `featured.length > 0`; replaces v11–v14 CategorySponsorBanner). Bicameral grid: left identity over right rail of 5-col rows (medallion · Company + `#{i+1}` rank chip (position, NOT stored `rank`) + scheme-prepended site link · Sales Contact `contact_name` · Phone+⧉ · Email+⧉). `<CircuitTraces variant="static" />`; always expanded; NO flashlight (perf). Chip `onKeyDown` gates on `e.target === e.currentTarget`. Phones via `@shared/utils/phone.formatPhone` (US→`(NNN) NNN-NNNN`; intl `+CC`→spaces) — DB normalized. Add/remove: `category-suppliers/{feature,unfeature}` or a Featured sponsor POST (auto-features).
