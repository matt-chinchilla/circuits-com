...
- **`useKeywordRequestModal` hook** (`frontend/src/public/hooks/useKeywordRequestModal.ts`, shipped 2026-05-16 in `5e0cc7b`): consolidates form state (name, companyName, email, message, selectedTier, submitting/submitted/formError) + `handleSubmit(keyword)` + `resetAfterClose()` for the keyword-sponsorship `<RequestModal>`. Hosting page owns ONLY the open/close state — `/keyword` tracks `modalKw: string | null`, `/keyword/:keyword` tracks `modalOpen: boolean`. Default `selectedTier` is `'gold'` (the design's recommended tier) so AT users land on a chosen radio option; reset on close moves it back to `'gold'` so a non-tier-card entry point (AvailabilityCheck, custom-keyword CTA) doesn't carry a stale Silver/Platinum choice forward. Mirror this pattern for any new form-modal shared across pages — extract form state to a hook, leave open/close at the call site.
- **`SponsorTier` Literal** (`api/app/schemas/forms.py`, shipped 2026-05-16): `Literal["silver","gold","platinum"]` is the canonical tier enum. `KeywordRequestForm.tier` uses it so bogus payloads 422 at Pydantic with an explicit `literal_error` (instead of silently embedding arbitrary strings into the notify-email body). `JoinForm.tier` is still bare `str | None` (predates this pattern) — if a new tier-style field lands on another form, prefer the Literal.
...
### Site-wide horizontal-overflow guard (2026-05-16)
`html, body { overflow-x: clip }` (in `frontend/src/shared/styles/global.scss`) is a defense-in-depth against horizontal page scroll. Any time a descendant exceeds viewport width (wide tables, oversized SVG backdrops, flex items that propagated min-content from a `<PartsTable>` child), `clip` truncates the document horizontally without creating a scrolling container. **Use `clip`, NOT `hidden`** — `clip` doesn't disturb `position: sticky` ancestors (Navbar, admin sidebar), `hidden` does. The companion fix lives at `pages/category/CategoryPage.module.scss .content`: `min-width: 0; width: 100%` breaks the flex min-content chain so internal `overflow-x: auto` containers (`.popularScroll`, `.tableWrap`) can actually clip wide tables. Without `width: 100%`, `align-items: stretch` was unreliable when descendants had a larger min-content than the flex container. New pages with tables should follow the same `min-width: 0; width: 100%` pattern on their `.content`-equivalent wrapper.
...
- **API container has `build: ./api` with NO volume mount** — same trap as the frontend. Edits to `api/app/{models,routes,services,db}/*.py` don't reach the running container via `docker compose restart api`. Use `docker compose up -d --build api` so the new code lands in a rebuilt image. Symptom when missed: `python -m app.db.seed` reports a successful seed but the data placement reflects the OLD seed code (hit this twice 2026-05-16 chasing the `_PART_CATALOG` subcategory-rekey).
- **`./deploy.sh --reseed` destructive scope** — TRUNCATEs `sponsors, category_suppliers, categories, suppliers CASCADE` on prod, which CASCADES to `users, parts, part_listings, price_breaks, revenue`. The `messages` table is NOT in the cascade chain and survives intact. Implication: any admin-UI-added rows (suppliers, parts) outside the `seed.py` source-of-truth ARE wiped; passwords changed via admin UI ARE wiped (re-seeded with default `admin` for matthew/mike/john). Only invoke when the seed is the authoritative dataset for parts/categories/suppliers — which it currently is.
