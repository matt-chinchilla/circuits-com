...
- **Sponsor tier↔placement matrix (2026-06-12 tier-boards)** — top-level cat (`parent_id IS NULL`)=**Platinum** only (single, supersedes peers); subcategory=**Gold** (single, supersedes Gold) **+ Silver** (multi directory); keyword=**Silver/Gold**. "Featured" MERGED→Platinum (gone; migration 013 rewrote the trigger + backfilled prod). 3-way: TRIGGER `sponsor_tier_placement_check` (rejects raw SQL), `_validate_tier_placement`→422, admin `choosePlacement`. Tier-aware supersede `_is_single_slot(tier,is_top_level)` expires same-tier peers (Platinum-top/Gold-child single; Silver+keyword multi). `UNIQUE(supplier_id,category_id|keyword)`→≤once; dup POST→409.
...
- **SponsorBlock = the **Gold** subcategory flashlight PCB card** (`SponsorBlock.tsx`, `data-tier="gold"`). `touch-action: none` + `setPointerCapture` (skip on interactive descendants). NO `whileHover` (FM fires on touch). PCB SVG `preserveAspectRatio="xMidYMid meet"` (NOT `slice`; portrait viewBox `0 0 300 360`). `mask-composite` rim behind `@supports`. Float-in ONCE/session.
- **Sponsor boards, one source `sponsors`** — **Platinum** `CategorySponsor.tsx` = top-level via `GET /api/categories/{slug}/partners` → `{slug,name,platinum: SponsorResponse|null}` (child→parent in `get_category_partners`; on parent+subpages; animated `<canvas>` tile-field engine `csFx.tsx` ported 1:1, **never alter the constants** GAP 19/shimmer .36/dome R 72/flip .34·255; Open-Placement/pitch + logo brand-takeover; GLOBAL `categorySponsor.scss` string-selectors not CSS-modules; `.cs-band` NO max-width/gutter — nested in `.contentWide`). **Gold** `SponsorBlock` (`category.sponsor`, subpage). **Silver** `SilverPartners.tsx` (`category.silver[]`, subpage, multi, electron-animated). `CategoryPartnersBanner.tsx` ALWAYS renders `<CategorySponsor>` (Open-Placement when null); `partnersMemo` (top-level slug, `bustSponsorCaches`-cleared) → sync; `/partners` `no-cache`+ETag/304. Layout: band → `.tierRow` (Silver main+Gold aside, SUBPAGES-ONLY `category.parent != null`) → full-width parts. DELETED `PreferredPartnersBanner`.
- **Subcats NESTED `/category/:parent/:child`; flat `/category/:slug` redirects** — both routes → one `CategoryPage`. Canonical redirect MUST be an EFFECT (`navigate(...,{replace})`), NOT render `<Navigate>` — `ErrorBoundary key={pathname}` drops the rendered child's nav-effect on remount (empty page, frozen URL). `?p`-reset effect bails while redirecting (`if (needsCanonicalRedirect) return`). URLs via `categoryPath(slug, parentSlug?)`; sitemap nested.- **`get_all_categories` keeps `featured_suppliers:[{id,name}]` + `featured_supplier_name`** (tier-ordered top sponsor per cat; admin tree READ-ONLY, manage via /admin/sponsors).
- **`<CircuitTraces variant="full" />` is hero-ONLY** — exists once, inside `BackdropLayer` (App.tsx, above `<Routes>`, persistent across nav). Every other uses `variant="static"` EXCEPT the Silver `SilverPartners` board (deliberate 2nd `full` — IO-gated + filter-free, 60fps-measured; the design wants its electrons). `full` ships 14 SMIL electron loops + 6s draw + IO/visibility/animationend bookkeeping; doubling them on a non-hero route is the Tier-3 #6 perf invariant (2026-04-19; v15 banner = chip-hover-lag smoking gun). See CircuitTraces.tsx:75-80.
...
- **`./deploy.sh` now restarts nginx automatically** (2026-06-02) — no more `--frontend` chase. Both `deploy_all` and `deploy_frontend` end with `restart nginx` so the cached upstream DNS gets refreshed. Still run `deploy-preflight` agent before every deploy. **Local `nginx/nginx.conf` self-heals** (`resolver 127.0.0.11 valid=10s` + variable `proxy_pass`): a local `up --build <svc>` rebuild's new container IP is re-resolved, no more stale-upstream 502 + no manual local nginx restart (prod `nginx.ssl.conf` still relies on the deploy restart).
