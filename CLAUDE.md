...
- **Sponsor placement = write-side supersede + read-side filter** — `admin_sponsors._supersede_existing_for_category(...)` runs on POST+PATCH ONLY for non-Featured tiers landing on a CHILD category (the single-slot "Subcategory Sponsor"); `category_service.get_category_detail` mirrors the predicate (newest-by-`created_at`) for the SponsorBlock sidebar. **Exempt: Featured top-level** (banner is multi-row, peers coexist) AND **keyword** placements (multi-sponsor per keyword) — neither supersedes.
- **Sponsor tier ↔ placement rule (2026-06-02, REVISED — supersedes the original same-day "Featured-only" rule)** — Featured attaches ONLY to a top-level category (`parent_id IS NULL`); Silver/Gold/Platinum attach to a CHILD category (Subcategory Sponsor) OR a keyword. Enforced by `_validate_tier_placement(db, tier, category_id)` in `admin_sponsors.py` — looks up `Category.parent_id`, 422s on Featured+child, Featured+keyword, OR non-Featured+top-level. Admin SponsorFormPage: Category-Sponsor button `disabled={tier!=='Featured'}`, Subcategory/Keyword buttons `disabled={tier==='Featured'}`; `choosePlacement` auto-bumps tier→Featured for top-category / →Gold for sub+keyword; tier-onChange auto-flips placement (Featured→top-category, else→subcategory).
- **Featured Sponsor side-effect → CategorySupplier upsert** — POST/PATCH with tier=Featured + top-level category_id calls `_upsert_category_supplier_featured(supplier, category)` which flips `is_featured=True` (idempotent) and assigns rank = max(rank for category) + 1. Non-Featured (child/keyword) does NOT upsert. Without this, a Featured sponsor would NOT appear on PreferredPartnersBanner (banner reads `category_suppliers.is_featured`, not `sponsors`) — the 2026-06-02 "Matt's Test invisible" bug.
- **DELETE reverses the Featured side-effect** — `delete_sponsor` does `db.delete + db.flush()` then `_unfeature_after_delete`: sets `CategorySupplier.is_featured=False` IFF no other **Active-or-NULL** Featured sponsor remains for (supplier, category) — coexist guard via `or_(Active, is_(None))` so Expired/Paused peers don't count. Mirrors `/unfeature` (flip flag, keep row). Without it a deleted Featured sponsor lingered on the banner forever — 2026-06-02 "deleted sponsorship still on the website" bug. Frontend `handleDelete` must NOT pre-unfeature; backend owns every delete path.
...
- **Admin sponsor form = 3-way segmented placement** (`top-category|subcategory|keyword`) — `placementDerivedRef` MUST reset on `[id]` change (separate `useEffect`) or cross-edit nav keeps prior bucket. All 3 buttons go through `choosePlacement(p)`. Gating per the REVISED 2026-06-02 rule: Category=`disabled={tier!=='Featured'}`, Subcategory+Keyword=`disabled={tier==='Featured'}`. Top-level + subcategory dropdowns are a custom `IconSelect` listbox (renders Phosphor `<Icon>` per row — native `<select>` strips child markup) w/ container-focus a11y pattern (`aria-activedescendant` + per-option `id`, Home/End/Arrow nav). Tier `<select>` carries `data-tier` → shiny-purple Featured / matte Platinum-Gold-Silver badges (`TIER_OPTION_STYLE` map + `.selectWrap[data-tier]`).
...
- **Sponsor `tier` casing** — admin emits TitleCase (`'Featured'`), legacy seed lowercase (`'gold'`). TS type `tier: string` (no enum). Lowercase in JSX for `[data-tier='X']`. Server-side comparisons use `_is_featured(tier)` helper in `admin_sponsors.py` (`(tier or '').strip().lower() == 'featured'`) — never `tier == 'Featured'` literal.
...
- **PreferredPartnersBanner = the v15 partner directory** (`pages/category/components/PreferredPartnersBanner.tsx`, mounted on every category page where `featured.length > 0`). Replaces v11–v14 CategorySponsorBanner. Bicameral grid (`minmax(258px,312px) 1fr`): left `.fbId` (kicker + count + tagline + CTA) over right `.fbRail` (silkscreen column header + vertical `.fbStackScroll` of 5-col `.fbChip` rows). Row = medallion · Company (+ `#{s.rank}` chip + scheme-prepended `.fbSiteLink`) · Sales Contact (`contact_name`, no title) · Phone+⧉ · Email+⧉. Vertical copper bus in left gutter (`.fbBusLine` + per-row `.fbVia`). `<CircuitTraces variant="static" />` paints gold-tinted PCB lattice behind via `--theme-pcb-trace` override. Always expanded; `max-height: 360px` scroll cap. NO flashlight (perf regression history). Chip `onKeyDown` MUST gate on `e.target === e.currentTarget` to preserve inner `<a>`/`<button>` keyboard activation. Add/remove path: `POST /api/admin/category-suppliers/{feature,unfeature}` (idempotent pair; unfeature flips `is_featured=False` without deleting the row) OR `POST /api/admin/sponsors/` with tier=Featured (side-effect auto-features the supplier).
- **Category page has TWO sponsor surfaces — DIFFERENT data sources** — (1) `PreferredPartnersBanner` reads `category.suppliers` filtered to `is_featured=true` (multi-row, all Featured CategorySuppliers). (2) `SponsorBlock` (right sidebar) reads `category.sponsor` — the newest visible row from the `sponsors` TABLE filtered `or_(status=='Active', status.is_(None))`. The two surfaces show DIFFERENT companies if the same admin add doesn't write to both — exactly the 2026-06-02 "Matt's Test invisible" bug class. Featured Sponsor POST now writes to both via `_upsert_category_supplier_featured`; non-Featured Sponsor (keyword-only) writes only to `sponsors`.
- **`get_all_categories` exposes `featured_suppliers: [{id,name}]`** (rank ASC) on every category + child, alongside legacy `featured_supplier_name` (= `[0].name`). The list carries the supplier `id` so the admin `/admin/categories` tree's per-row **Unfeature** button (→ `unfeatureSupplierInCategory`) targets the exact `CategorySupplier` row — names alone collide (`Supplier.name` has no unique constraint). Tree renders ALL featured suppliers (was a single hardcoded `featured_supplier_name` line). Both `featured_supplier_name` AND `featured_suppliers` are REAL Pydantic fields on `CategoryResponse`/`SubcategoryResponse` (not stamped attrs) so `response_model` keeps them.
- **`<CircuitTraces variant="full" />` is hero-ONLY** — exists exactly once on the page, inside `BackdropLayer` (mounted at App.tsx above `<Routes>`, persistent across public-route nav). Every other consumer (PreferredPartnersBanner, Category, Search, Contact, Join, About, KeywordSponsor, Part) uses `variant="static"`. `full` ships 14 SMIL animateMotion electron loops + 6s draw-circuit + IntersectionObserver/visibilitychange/animationend bookkeeping; doubling those on a non-hero route is the Tier-3 #6 perf invariant the 2026-04-19 fix removed. Documented in CircuitTraces.tsx:75-80 and the 2026-06-02 v15 code-review (the banner once mounted `variant="full"` and was the smoking gun for chip-hover lag).
- **`mix-blend-mode: screen` + per-pointermove CSS-var updates = full-banner CPU recomposite** — the gradient center moves per frame, so the compositor cannot cache the blend pass. `mask-image` on a sibling element compounds it (mask layer is uncacheable under the blend). For warm-pool ambient effects: pre-baked rgba radial-gradient with `will-change: background` stays GPU-fast; bake fade overlays into stacked `background:` linear-gradients instead of `mask-image`. History: deleted twice — v11.2 CategorySponsorBanner cursor-lamp + v15 PreferredPartnersBanner `useFlashlight` lamp. Net delete won both times. Do not reintroduce.
...
- **Prod one-shot data fix pattern** (SQL with apostrophes/HEREDOC escape hell): `Write tmp/fix.sql → aws ec2-instance-connect send-ssh-public-key → scp file to ec2 → ssh + sudo docker cp to db container → docker exec ... psql -U circuits -d circuits -f /tmp/fix.sql`. Avoids the triple-nested `\''` escape soup of inline `ssh "psql <<SQL ... SQL"`. Instance Connect SSH key is 60-second TTL — push it again right before each `ssh`/`scp` invocation.
