...
- **All routes except `HomePage` lazy-load** via `React.lazy()` + `<Suspense fallback={<RouteFallback />}>` in `App.tsx` (2026-04-19 Tier 3). Main bundle is 262 KB; Recharts (~400 KB) lives in the admin chunk only, never ships to public visitors. New heavy vendor deps go in `vite.config.ts` → `build.rollupOptions.output.manualChunks` (recharts/framer/router already isolated). Keep Home eager for LCP.
...
The `<defs>` glow filter was REMOVED 2026-04-19 (Tier 2 perf): at `stdDeviation=0.5` it was visually negligible but re-rasterized the electron subtree per frame. `<CircuitTraces variant={'full' | 'static'}>` — Home uses `'full'` (animated electrons + draw-in animation); the 7 inner pages (Category, Search, Contact, Join, About, KeywordSponsor, Part) pass `variant='static'` (no electrons, no draw animation, no useEffect, zero GPU in steady state). Alternate themes (steel/schematic/pcb) inherit `background-color: var(--theme-nav-bg)` on `.hero` so the board color matches the navbar — base keeps `$executive-blue`. Pause wiring in CircuitTraces.tsx useEffect: `IntersectionObserver` + `visibilitychange` pause all animations when off-screen or tab-hidden (`svg.pauseAnimations()` for SMIL + `data-paused="true"` attribute for CSS, handled by `.circuitTraces[data-paused="true"] .trace` selector); an `animationend` handler releases completed `draw-circuit` animations via `el.style.animation = 'none'` (after pinning `stroke-dashoffset: 0` to preserve the drawn end-state). Check this pattern before adding new animation work.
...
- **About page has its own `.hero` class** (separate from `HeroSection.module.scss`) that hardcodes `#0a4a2e` (executive-blue) instead of `var(--theme-nav-bg)` — it paints the base-theme color on every theme. Caught 2026-04-19 by the `theme-persistency-guard` agent dry-run; deferred fix tracked in memory `project_hero_cross_page_gap`. When changing the cross-page theme system, verify this page's hero is updated or explicitly excluded.
