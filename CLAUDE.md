...
- **PageHeaderBand owns its themed dark bg** — relying on PublicLayout's persistent backdrop to show through is wrong: each page wrapper (`.aboutPage`, `.contactPage`, `.joinPage > .page`) sets `background: var(--theme-surface-bg)` (light) which covers the backdrop entirely. Components needing a dark themed surface must provide it directly (per-theme `:global([data-theme="x"])` overrides). Caught 2026-04-25 (commit c5eb07f); same trap will catch any new overlay component built on top of PublicLayout.
