...
- **`<CircuitTraces variant="full" />` is hero-ONLY** — exists once, inside `BackdropLayer` (App.tsx, above `<Routes>`, persistent across nav). Every other uses `variant="static"` EXCEPT the Silver `SilverPartners` board (2nd `full`, IO-gated + filter-free, 60fps). `full` = 14 SMIL loops + 6s draw + IO bookkeeping; a 2nd on a non-hero route = perf invariant.
