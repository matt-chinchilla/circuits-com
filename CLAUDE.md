...
- **Logo crop + sponsor brand colors (2026-07-10)** — `@shared/components/LogoCropperModal` (portal dialog, pan/zoom/arrow-keys, focus-trapped; **geometry authority = MEASURED `frameSize`** via clientWidth+ResizeObserver, NEVER the 320 const — CSS caps the frame on mobile and the crop must match what's visible) hosts BOTH upload paths (admin `ImageUploadField` + Platinum drag-n-drop demo). Brand colors: `Sponsor.brand_primary/secondary` (String(9), migration 018, `validate_optional_hex_color`→422) OVERRIDE the `Supplier.brand_*` fallback; public payloads stamp resolved colors + **`brand_takeover`** (true ONLY for sponsor-level colors) at BOTH construction sites (`_sponsor_board_dict` + keyword route, `x if supplier else None` guards). Sold board auto-brands from FIRST PAINT on `brand_takeover` (known cosmetic: ~100ms steel flash on cold load — async `/partners`), logo-pad click still toggles; a same-values refetch must not stomp the toggle (primitive-dep sync effect `[sponsor?.id, sponsor?.brand_takeover]`). Extraction rehomed to `@shared/utils/brandPalette` (`extractBrandPalette` — 1:1 parity w/ the old csFx algorithm + ranked `swatches`; `mixHex` replaces `color-mix()` strings); picker = `@shared/components/BrandColorPicker` (admin form `allowCustom` + public pitch `compact`). **Round 2 (2026-07-11):** `Supplier.brand_primary/secondary` are now API-settable (create/update, same `validate_optional_hex_color`) via the admin supplier form — no longer seed/fallback-only — AND on save the supplier form propagates the pair onto that supplier's ACTIVE Platinum sponsorship(s) (`Promise.allSettled` PATCH, `normalizeSponsorTier`/`isActiveSponsor(s.status)`-filtered, BEFORE toast/navigate) so the sold board picks up `brand_takeover` without a second edit; supplier colors still ALSO serve as the read-side fallback. A two-step `@shared/components/BrandColorSelectModal` (%-ranked swatches + `nearestCssColor` names + Primary/Secondary radios + custom hex; Skip keeps existing/auto colors) now gates EVERY crop — sponsor form, supplier form, AND the Platinum demo (which persists a PROVISIONAL auto-color pitch before the modal so reload keeps the crop). **`safeHexColor` (`@shared/utils/color`) gates EVERY stored-hex→inline-style path** (mirror of safeImageUrl). Sponsor form requires BOTH colors or NEITHER (half-set = takeover w/ surprise default channel). Demo pitch stores the SMALL cropped data-URL in sessionStorage (quota fix); encode failure surfaces via `.csb-croperr` aria-live node (never silent).
