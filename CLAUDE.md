...
- **Featured Sponsor side-effect → CategorySupplier upsert** — POST/PATCH with tier=Featured + top-level category_id calls `_upsert_category_supplier_featured(supplier, category)` which flips `is_featured=True` (idempotent) and assigns rank = max(rank among `is_featured` rows) + 1 — non-featured rows must not inflate it (2026-06-03 Pasternack-#7). Non-Featured (child/keyword) does NOT upsert. Without it a Featured sponsor wouldn't reach PreferredPartnersBanner (reads `category_suppliers.is_featured`) — 2026-06-02 "Matt's Test invisible" bug.
...
- **SponsorBlock = subcategory backlit-PCB card with flashlight reveal** (`SponsorBlock.tsx`). `touch-action: none` + `setPointerCapture`; skip capture when target is an interactive descendant. NO `whileHover` (FM fires on touch). PCB art SVG = `preserveAspectRatio="xMidYMid meet"` (NOT `slice` — portrait viewBox `0 0 300 360`; `slice` overflows). `mask-composite` rim gated behind `@supports`.
- **PreferredPartnersBanner = the v15 partner directory** (`pages/category/components/PreferredPartnersBanner.tsx`, mounted where `featured.length > 0`). Bicameral grid: left identity + right rail of 5-col rows (medallion · Company + `#{i+1}` rank chip (position, NOT stored `rank`) + scheme-prepended site link · `contact_name` · Phone+⧉ · Email+⧉). `variant="static"`; always expanded; NO flashlight (perf). Chip `onKeyDown` gates on `e.target === e.currentTarget`. Phones via `@shared/utils/phone.formatPhone` (DB normalized). Add/remove: `category-suppliers/{feature,unfeature}` or a Featured sponsor POST (auto-features).
- **Category `.suppliers` = the category's OWN rows ONLY (no child rollup)** — `get_category_by_slug` uses `cat_ids=[category.id]`, NOT children, so a child's Featured Subcategory Sponsor does NOT surface on the PARENT banner/TopPartners (2026-06-02 "Oneonta still on PMICs via ldo-regulators rollup" bug). PARTS still roll up; suppliers do not.
- **Subcats are NESTED `/category/:parent/:child`; flat `/category/:slug` redirects to nested** — `App.tsx` has BOTH routes → one `CategoryPage` (fetches by the globally-unique child slug). Canonical redirect (flat-child or wrong-parent → nested) MUST be an EFFECT (`navigate(...,{replace})`), NOT a render `<Navigate>` — `PublicLayout`'s `<ErrorBoundary key={pathname}>` drops the rendered child's nav-effect on the keyed remount (page rendered empty, URL frozen — 2026-06-03). AND the `?p`-reset effect must bail while redirecting (`if (needsCanonicalRedirect) return`) or its `setSearchParams` reverts it to flat. URLs via `@shared/utils/categoryPath(slug, parentSlug?)`; `index.html` preload regex grabs the LAST segment; sitemap nested for children.
- **Category page has TWO sponsor surfaces — DIFFERENT data sources** — (1) `PreferredPartnersBanner` reads `category.suppliers` filtered to `is_featured=true`. (2) `SponsorBlock` reads `category.sponsor` — the newest visible `sponsors`-TABLE row (`or_(status=='Active', status.is_(None))`). The two diverge if an admin add doesn't write both — the 2026-06-02 "Matt's Test invisible" bug. Featured POST writes both (`_upsert_category_supplier_featured`); non-Featured (keyword) writes only `sponsors`.
- **`get_all_categories` exposes `featured_suppliers: [{id,name}]`** (rank ASC) on every category + child, alongside legacy `featured_supplier_name` (= `[0].name`). The list carries the supplier `id` so the admin `/admin/categories` tree's per-row **Unfeature** button (→ `unfeatureSupplierInCategory`) targets the exact `CategorySupplier` row (names collide — no unique constraint). Tree renders ALL featured suppliers. Both are REAL Pydantic fields (not stamped attrs) so `response_model` keeps them.
- **`<CircuitTraces variant="full" />` is hero-ONLY** — exists once, inside `BackdropLayer` (App.tsx, above `<Routes>`, persistent across nav). Every other consumer uses `variant="static"`. `full` ships 14 SMIL electron loops + 6s draw + IO/visibility/animationend bookkeeping; doubling them on a non-hero route is the Tier-3 #6 perf invariant (2026-04-19; v15 banner = chip-hover-lag smoking gun). See CircuitTraces.tsx:75-80.
