...
- **SponsorBlock = subcategory backlit-PCB card with flashlight reveal** (`SponsorBlock.tsx`). `touch-action: none` + `setPointerCapture`; skip capture when target is an interactive descendant. NO `whileHover` (FM fires on touch). PCB art SVG = `preserveAspectRatio="xMidYMid meet"` (NOT `slice` — portrait viewBox `0 0 300 360`; `slice` overflows). `mask-composite` rim gated behind `@supports`. Float-in plays ONCE/session (module flag), else `initial={false}`.
- **PreferredPartnersBanner** (`pages/category/components/PreferredPartnersBanner.tsx`) — bicameral grid (left identity + right rail of supplier rows). `variant="static"`, always expanded, NO flashlight (perf). Mounted by the thin `CategoryPartnersBanner.tsx` wrapper INSIDE CategoryPage (top of `.contentWide`, BELOW breadcrumb + sub-nav — NOT in PublicLayout, which renders above the nav; hookify-guarded).
- **The banner is a TOP-LEVEL artifact via `GET /api/categories/{slug}/partners`** (a child slug resolves to its parent in `get_category_partners`) → SAME banner on parent + every subpage. `CategoryDetailResponse` has NO `suppliers` (`SponsorBlock`'s `category.sponsor` stays). Session memo `@shared/services/partnersMemo.ts` (keyed top-level slug, cleared by `bustSponsorCaches`) → sync render. `/partners` = `no-cache` + ETag/304, SW-cached `api-categories`.
...
- **Two banner surfaces, ONE source (`sponsors`)** — Preferred Partners (top-level, via `/partners`) + `SponsorBlock` (newest sponsor for THIS subcat, `category.sponsor`); both filter `or_(status=='Active', status.is_(None))`. "Top Distributors"/`TopPartners` deleted.
...
- **`--reseed` can DEADLOCK when the deploy ships a heavy-DDL migration** — `deploy_reseed` recreates the api (re-runs alembic+seed) THEN `TRUNCATE`s; a migration's table DDL (DROP COLUMN/ADD CONSTRAINT/CREATE TRIGGER = AccessExclusiveLock) racing the TRUNCATE → `deadlock detected`, TRUNCATE aborts, old data persists (migration 011). Recovery (api now UP+idle): re-run the clear+reseed alone (`docker exec …db psql -c 'TRUNCATE …CASCADE'` then `docker exec …api python -m app.db.seed`).
...
- **Edge gzip = `nginx/nginx.ssl.conf`** (`gzip on`/`gzip_proxied any`/`json`+`javascript`) — was OFF (drift; the inner `frontend/nginx.conf` gzip skips edge-proxied upstreams). Cold mobile cat load 639→189 KB. Guard `test_nginx_gzip.py`. **alembic 012** = hot-column indexes (`parts.category_id`/`sub_slug`, `part_listings.part_id`, `price_breaks.listing_id`/`min_quantity`), index-only; guard `test_part_indexes.py`.
