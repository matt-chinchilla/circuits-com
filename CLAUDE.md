...
### Theme System
4 themes (`base`, `steel`, `schematic`, `pcb`) defined in `frontend/src/shared/styles/_themes.scss` as CSS custom properties (`--theme-*`) scoped to `[data-theme]` on `<html>`. **Steel is the prod default** — `ThemeBridge.tsx` hardcodes `theme = "steel"` in prod (URL params + localStorage ignored so stale state can't strand users on a non-steel theme). In dev, full resolution: URL `?nav=A|B|C` → `localStorage.circuits.nav.theme` → `DEFAULT_THEME` constant (`"steel"`). `NavVariantPicker.tsx` is the floating preview pill — **dev-only**, gated by `{import.meta.env.DEV && <NavVariantPicker />}` in `App.tsx` (mirrors `HeroColorTuner`). `localStorage.setItem` is also dev-only in ThemeBridge so prod never writes the theme key. Adding a theme = new block in `_themes.scss` + entry in `KEY_TO_THEME` (ThemeBridge) + `VARIANTS` (picker). The theme cascades site-wide because every site-level accent uses `var(--theme-accent)` / `var(--theme-cta-bg)` instead of `$nav-blue` / `$executive-blue` directly.
...
- **Breakpoint `$bp-desktop: 1199px`** exists in `frontend/src/shared/styles/_variables.scss` alongside `$bp-mobile: 768px` and `$bp-tablet: 1024px` — use `@include responsive($bp-desktop)` rather than hardcoding widths. Two admin-only mobile breakpoints (added 2026-05-13): `$bp-admin-mobile: 820px` (sidebar→drawer flip + topbar reflow + content collapse) and `$bp-admin-compact: 420px` (further compaction — `.pageTitle` 16→15px, `.demoState` ON/OFF text hides, KPI rows → 1 col). Reuse `$bp-mobile` for the website hamburger threshold and `$bp-tablet` for the admin first-collapse — don't introduce `$bp-website-mobile` or `$bp-admin-tablet`, they'd just shadow existing vars.
- **Mobile drawer state-machine pattern** (`Navbar.tsx` + `AdminLayout.tsx`, post 2026-05-13): `useState(menuOpen)` + three effects keyed on `[menuOpen]` for body-scroll-lock (capture `prev`, restore on cleanup), Esc keydown (attach listener only while open via `if (!menuOpen) return undefined`), and `[location.pathname]` for route-change auto-close. Drawer link `onClick` calls `setMenuOpen(false)` BEFORE `<NavLink>` handles navigation in the same tick. Apply `.isOpen` class on burger/scrim/drawer/aside (admin only) and add `aria-expanded` on burger + `aria-controls` referencing drawer id. Animations use compositor-only `transform` + `opacity` (no filter/drop-shadow); a `@media (prefers-reduced-motion: reduce)` block resets transitions to `none`. For the two callers (one public, one admin) the spec deliberately declined a shared `useMobileDrawerState` hook — extraction is over-engineering at the 2-caller threshold (note: a 3rd Esc-handler exists in `ConfirmDialog` + a 4th in `AdminLayout`'s Cmd+K block — consolidating all 4 into `useEscKey` is sensible IF a third drawer surface ever lands).
...
- **Bulk `query(...).delete(synchronize_session=False)` + `lazy="selectin"` relationship = SQLAlchemy "tried to blank-out primary key" crash on the parent delete.** The selectin loader pre-loads the related rows into the session; the bulk delete strips them from the DB but NOT from the session; then `db.delete(parent)` walks the still-loaded relationship and tries to NULL the FK column on stale session objects. If that FK is part of a composite PK (e.g., `CategorySupplier.supplier_id`), SQLAlchemy raises `AssertionError: Dependency rule on column 'X' tried to blank-out primary key column 'Y'`. **Fix**: insert `db.expire(parent)` (or `db.expire_all()`) AFTER the bulk deletes and BEFORE `db.delete(parent)` — invalidates the session cache so the parent delete reads fresh state. See `routes/suppliers.py:delete_supplier` for the canonical example. The `lazy="selectin"` setting on `Supplier.category_associations` was the trigger.
- **Admin sponsor store is now centralized at `frontend/src/admin/services/sponsorStore.ts`.** Both list + form pages route through `loadSponsors / findSponsor / upsertSponsor / deleteSponsor` — no page-local `STORE_KEY` / `readStore` / `writeStore` anymore. Critical behavior: `loadSponsors()` materializes `SEED_SPONSORS` into localStorage on the very first read (when the key is absent), so deleting a seed sponsor (e.g. `spn-honeywell-sensors`) actually removes one row instead of writing `[]` and wiping the entire seed list. Pre-2026-05-05 the list-page `loadStore()` returned `SEED_SPONSORS` as an in-memory fallback while the form-page `readStore()` returned `[]` — that mismatch was the root cause of the "delete-button doesn't work" bug class on seed-only sponsors.
- **`response_model=SchemaX` silently strips computed attributes that aren't on SchemaX.** When a FastAPI route has `response_model=list[SupplierResponse]` and you stamp `obj.parts_count = N` on the ORM rows before returning, Pydantic's `from_attributes=True` will only read attributes declared on the schema — extras are dropped. The route "computes correctly" but the client receives a stripped payload, and the frontend shows `?? 0` defaults forever. **Symptom**: a list endpoint shows zeros while the matching detail endpoint shows real values (because detail typically returns a plain dict and bypasses the schema). **Fix**: either drop `response_model=` and return dicts (mirroring `get_supplier`), or extend the schema with optional fields + defaults. The 2026-05-05 supplier-list zero-counts bug was this exact pattern — `routes/suppliers.py:list_suppliers` used `response_model=list[SupplierResponse]` and `SupplierResponse` had no `parts_count`/`categories`, so even computed values were silently dropped. Tests for the *detail* endpoint (`/api/suppliers/{id}`) covered `parts_count` but no test asserted the *list* endpoint included it — recurring "tested adjacent, missed canonical" gap. Regression test added in `tests/test_suppliers_extended.py::TestListSuppliersAggregates`.
- **SMTP creds for forms live in `.env` on the prod EC2 host** — not committed. Populate `SMTP_HOST=mail.hover.com`, `SMTP_PORT=587`, `SMTP_USERNAME=no-reply@circuits.com`, `SMTP_PASSWORD=<from Hover admin>` in `/opt/circuits-com/.env` next to `docker-compose.prod.yml`. Without `SMTP_HOST` set, `app.services.email._smtp_send` runs in demo mode and logs the message instead of sending — local dev works without leaking creds. `NOTIFY_RECIPIENTS` defaults to `["no-reply@circuits.com"]` (loopback for testing); set the env var to `john@circuits.com,mike@circuits.com` (comma-separated — `Settings._split_csv` validator handles both JSON and CSV) when ready to flip.
- **n8n is no longer in the form-submission path** — `routes/forms.py` (post-2026-05-08) sends emails directly via `app.services.email` (aiosmtplib + Hover SMTP) using FastAPI `BackgroundTasks`. The n8n container stays in `docker-compose.yml` for future workflow needs but the webhook fire was removed because workflows were never auto-imported and SMTP creds were never wired. If you re-add n8n for a workflow, import it via the n8n CLI on first boot — copying JSON to `/home/node/.n8n/workflows/` does not auto-load.
...
- **`var(--executive-blue)` is the project's "WCAG-legible accent text on light surfaces" token — recurring pattern.** Any `var(--theme-accent)` or `var(--theme-cta-bg)` used for *text* on a white/light-tint background fails contrast under at least one theme: `--theme-accent` is `#44bd13` (bright green, ~2.5:1 on white) or `#c49b5d` (copper); `--theme-cta-bg` is `#f4f5f7` near-white under steel. Three pages have hit this so far — `.contactLine:hover` (2026-04-25), `.contactReason.on` (2026-05-11), and 5 spots on PrivacyPage (`.tocItemNum`, active `.tocItemTitle`, `.sectionNum`, `.licenseTag`, `.tocFootLink` — 2026-05-12). **Rule: any small accent-colored text on a light card uses `var(--executive-blue)` (always #0a4a2e, WCAG-AA across all 4 themes). Borders and chip backgrounds can keep `var(--theme-accent)` — only the text needs the swap.** Sub-gotcha: `var(--fg2)` (#6b7280) passes 4.85:1 on white but fails 4.07:1 on `var(--surface)` (#eef1f5) — the bg differential matters at this exact grey. Lighthouse a11y audit will catch the bg-specific variant.
