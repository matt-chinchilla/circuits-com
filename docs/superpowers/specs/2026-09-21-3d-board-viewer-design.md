...
`quality = webgl2 ? 'full' : 'reduced'` (AMENDED 2026-09-23: the `innerWidth >= 900` gate is gone — the owner on a phone saw no component bodies; phones now get `full`, measured Glasgow at 390×844×3: 308,634 triangles / 12 draw calls / 740×828 backing store. AMENDED 2026-09-22: the original `&& devicePixelRatio <= 2` gate put the owner's 2.24×/2.52× desktop displays — an RTX 4070 Ti — on the reduced tier: 1× backing store, no bodies; the pixel cost is bounded by the renderer's `setPixelRatio(min(dpr, 2))` instead), evaluated once at mount and stored in state; a module-level `setQualityOverride('full' | 'reduced' | null)` (exported for tests and for the day the owner wants "the same everywhere") wins over the heuristic. `reduced`: `renderer.setPixelRatio(1)`, no courtyard bodies, tracks narrower than 0.2 mm skipped, arc tolerance 0.05 mm, drill polygons 8-gon. `full`: pixel ratio `min(devicePixelRatio, 2)`, everything. Draw calls: one mesh per `MeshGroup` (Glasgow ≈ substrate 1 + walls 1 + copper 4 + mask 2 + silk 2 + bodies 1 = ~11).
